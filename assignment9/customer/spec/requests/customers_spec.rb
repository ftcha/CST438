...
RSpec.describe 'CustomersController', type: :request do
    ...
    it 'duplicate email' do
        headers = { 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' } 
        Customer.create( firstName: 'Blaze', lastName: 'King', email: 'blazeking@gmail.com' )
        customer2 = {'firstName' => 'Blaze', 'lastName' => 'King', 'email' => 'blazeking@gmail.com' }
    ...
    it 'get by id' do
        headers = { "CONTENT_TYPE" => "application/json", 'ACCEPT' => 'application/json' }
        customer = Customer.create( firstName: 'Blaze', lastName: 'King', email: 'blazeking@gmail.com' )
    ...
    it 'get by email' do
        ...
        customer = { 'firstName' => 'Blaze', 'lastName' => 'King', 'email' => 'blazeking@gmail.com' }
        customer = Customer.create( firstName: 'Blaze', lastName: 'King', email: 'blazeking@gmail.com' )
        get '/customers?email=blazeking@gmail.com', :headers => headers
        expect(response).to have_http_status(200)
        customer_response = JSON.parse(response.body)
        expect(customer_response['id']).to eq customer.id
        expect(customer_response['email']).to eq customer.email
    ...
    it 'testing invalid email' do
        headers = { 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' } 
        get '/customers?email=none@none.com', :headers => headers
        expect(response).to have_http_status(404)
    end
    
    it 'testing award' do
        headers = { 'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json' } 
        customer = Customer.create( firstName: 'Blaze', lastName: 'King', email: 'blazeking@gmail.com', lastOrder: 0, lastOrder2: 0, lastOrder3: 0, award: 0)
        ...
        expect(response).to have_http_status(200)
        customer_response = JSON.parse(response.body)
        expect(customer_response['award']).to be_within(0.01).of(28.67)
